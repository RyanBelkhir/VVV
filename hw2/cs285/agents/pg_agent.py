...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            ...
            for reward in rewards_list:
                values.append(self._discounted_return(reward))
            q_values = np.concatenate(values)
        # Case 2: reward-to-go PG
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
        else:
            values = []
            for reward in rewards_list:
                values.append(self._discounted_cumsum(reward))
            q_values = np.concatenate(values)
        return q_values
    ...
    def estimate_advantage(self, obs, rews_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            assert values_unnormalized.mean() == q_values.mean()
            assert np.std(values_unnormalized) == np.std(q_values)
            values = (values_unnormalized * np.std(values_unnormalized)) + np.mean(values_unnormalized)
    ...
    def _discounted_return(self, rewards):
        ...
        t_prime = np.arange(len(rewards))
        gammas = np.power(self.gamma, t_prime)

        discounted_return = gammas * rewards
        list_of_discounted_returns = np.ones_like(rewards) * np.sum(discounted_return)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for t in range(len(rewards)):
            t_prime = np.arange(t, len(rewards))
            discounts = np.power(self.gamma, t_prime - t)
            discounted_cumsum = discounts * rewards[t:]
            sum_discounted_cumsum = np.sum(discounted_cumsum)
            all_discounted_cumsums.append(sum_discounted_cumsum)
